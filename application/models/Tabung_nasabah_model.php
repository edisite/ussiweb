<?php
class Tabung_nasabah_model extends Grocery_crud_model
{
    ...
    function get_list()
    {
        ...
        $select .= ", nasabah.nama_nasabah, tabung.kode_integrasi, tabung.kode_produk, tabung.kode_kantor,";
        ...
        $this->db->from($this->table_name.',tabung,nasabah');
        $this->db->where('tabung.no_rekening='.$this->table_name.'.no_rekening');
        $this->db->where('tabung.NASABAH_ID=nasabah.NASABAH_ID');
        $this->db->where('my_kode_trans<>300');
        $this->db->where('my_kode_trans<>400');
        $this->db->where('(upper(tabung.no_rekening) like "%%"');
