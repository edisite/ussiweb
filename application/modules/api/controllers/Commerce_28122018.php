...
//            $this->response($data); 
    ...
    protected function _Check_agent_topup($in_agent_id = null,$error_code,$trace_id = '',$arr_custom = ''){
        if($this->Admin_user2_model->User_by_id($in_agent_id)){            
        }else{ 
    ...
    function _Check_prm_bpjs($var = '',$errorcode = '',$trace_id = '',$field = '',$data = '') {
        if(empty($var)){
            $arr_res   = array('status' => FALSE,'error_code' => $errorcode,'message' => $field,'data' => array('bpjsid' => '','cust_name' => '','periode' => '','tagihan' => '','adm' => '','total' => '','code' => ''));
            $this->logAction('response', $trace_id, $arr_res, 'failed,'.$field);
            $this->response($arr_res);            
        }
    }
